...
function SideMenuItem({
  ...
  switch (context) {
    ...
    case 'repository':
      checkContext = checkOrg() && checkProd() && checkRepo();
      break;
  ...
  if (optype === 'drawer' || checkContext) {
    return (
      <Box onClick={handleClose} ref={itemRef} sx={{ marginY: '2px' }}>
        <Tooltip title={tooltip} arrow placement="left">
          <Styles.Wrapper $collapsed={isCollapsed} onClick={handlePath}>
        ...
        {optype === 'drawer' && (
          <Drawer anchor="left" open={openState} onClose={handleClose}>
            {children}
          </Drawer>
        )}
      </Box>
